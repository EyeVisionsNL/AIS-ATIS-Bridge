...
DEFAULTS={"receiver":"","tuning_mode":"scan","selected_channel_id":"v61_botlek","channel_bank":"rotterdam_port","channels":[{"id":i,"label":n,"frequency_mhz":f,"scan_enabled":e} for i,n,f,e in DEFAULT_CHANNELS],"gain_mode":"auto","gain_db":20.7,"ppm":0,"ais_ships_url":"http://127.0.0.1:8119/ships.json","ais_viewer_url":"http://127.0.0.1:8119/","web_host":"0.0.0.0","web_port":8120}
...
def validate(raw:dict[str,Any])->dict[str,Any]:
    ...
    result["tuning_mode"]="fixed" if raw.get("tuning_mode")=="fixed" else "scan"; result["channel_bank"]=str(raw.get("channel_bank") or "marine")[:48]
